...
### 2屏幕正中间有个元素A，元素A中有文字A，随着屏幕宽度的增加，始终需要满足下列条件
```js
/* 
  A元素垂直居中于屏幕中央
  A元素距离屏幕左右边距各10px
...
### 3列举出 css 选择器有哪些分类，并至少写出三个 css 选择器之间的区别，适用场景



### 4Css 实现 div 宽度自适应，宽高保持等比缩放



### 5ul 内部除最后一个 li 以外设置右边框效果



### 6flex:1 的完整写法是？分别是什么意思？
...
### 9请画出 css 盒模型，基于盒模型的原理，说明相对定位、绝对定位、浮动实现样式是如何实现的？
...
### 10Css 实现多列等高布局，要求元素实际占用的高度以多列中较高的为准
...
### 12说下盒模型的区别？介绍一下标准的 CSS 盒模型？border-box 和 content-box 有什么区别？
...
### 14一个标签的 class 样式的渲染顺序，id、class、标签、伪类的优先级
...
### 18居中为什么要使用 transform（为什么不使用 marginLeft/marginTop）
...
### 22transform 动画和直接使用 left、top 改变位置有什么优缺点
...
### 26动手实现一个左右固定100px，中间自适应的三列布局？(至少三种)
...
### 30flex 布局，如何实现把八个元素分两行摆放



### 31Css 方式实现一个不知道宽高的 div 居中都有哪几种方法
...
### 34屏幕占满和未占满的情况下，使 footer 固定在底部，尽量多种方法
...
### 36Css 超出省略怎么写，三行超出省略怎么写



### 37Css inherit、initial、unset 三者的区别



### 38响应式布局用到的技术，移动端需要注意什么
...
### 39布局都有什么方式，float 和 position 有什么区别
...
### 41BFC 是什么？触发 BFC 的条件是什么？有哪些应用场景？
...
### 42通过 link 进来的 css 会阻塞页面渲染吗，Js 会阻塞吗，如果会如何解决？
...
### 48position 定位都有什么属性（不仅仅是绝对定位和相对定位/fix 定位）
